...
- 采用dfs或bfs遍历图并记录顶点数量，最大的连通量就是最大连通子图
...
- 浮点数表示：符号位（1位）、指数位（8位或11位）、尾数位（23位或52位）
- 符号位：0是正数，1是负数
- 指数位：偏移表示法（实际值-127/1023）
- 尾数位：[1,2)
...
- 容量小
- 层级结构
- 写回和写直达：写回（只在cache中修改数据，并在将其写回主存之前保存已修改的数据）；写直达（立即将数据写回主存）
- 替换策略：LRU、FIFO

### 21、原子操作内存顺序

- 原子操作：不可分割的基本操作，要么完全执行，要么完全不执行，不会被中断
- **原子操作的执行顺序和内存访问顺序有关**
- 松散顺序：没有任何顺序限制
- memory_order_acquire：**保证在当前操作之前的所有读操作完成（获取锁）**
- memory_order_release：**保证在当前操作之后的所有写操作完成（释放锁）**
- memory_order_acq_rel：**原子操作增减**
- memory_order_seq_cst：**顺序一致（保证所有线程对共享变量的访问都是按照一个全局的顺序进行的）**

# 网易游戏测试一面

- **vector和list的区别：list底层deque实现、list不支持随机访问、插入和删除的效率、迭代器稳定性**
- url输入网址会发生什么：**解析url（**协议、主机名、路径）、**DNS解析**（将主机名发送到DNS服务器、查找对应ip）、**建立tcp连接**、**发送请求**、**服务器处理请求**、**浏览器渲染**、**关闭tcp连接**
- tcp和udp区别：**tcp（面向连接、提供可靠、面向字节的数据传输；使用确认、重传、流量控制等机制确保数据的完整性和可靠性；有序、拥塞控制）udp（无连接、不可靠、无序；实时性要求高，可以允许数据丢失）**
- 应用层协议
- 传输层协议
- **应用层协议哪些使用tcp（可靠、面向连接、需要确保数据的可靠传输），哪些使用udp（实时性高、可以容忍数据丢失）**：TCP（http、https、ftp、smtp、ssh：提供加密的远程登陆）、UDP（DNS、dhcp（动态分配ip地址）、tftp（简单文件传输）、volp（语音传输））
- **如何找到环形链表的入口：先检测是否有环，将快指针指向链表头部，相同速度移动两个指针，快慢指针相遇为入口**
- **内存分区**：代码段（机器代码）、数据段（全局变量、静态变量）、堆（动态分配）、栈（局部变量、函数参数、函数返回地址）
- **C++如何在走出作用域后执行某操作**：析构函数和智能指针
- **虚函数与普通函数相比的开销**：虚函数对象实例包含一个指向虚函数表的指针、访问速度、**编译器优化（内联、单一调用点优化：确定了实际的虚函数不需要二次查询虚函数表、静态绑定优化：确定了在某个上下文中使用的对象类型，可能会将虚函数转换为静态绑定，去虚化）**
- **大型项目如何分配测试任务**：功能模块、测试阶段（初期的功能测试、多人场景测试、本地化测试）、平台和设备、自动化测试和手动测试（依据场景判断）
- **游戏角色太强如何判断**：游戏用户的数据分析（胜率、登顶次数、前四占比。。。）、玩家反馈、内测和评估
- **游戏用户和游戏测试关注的区别**：游戏用户（游戏体验、沉浸感、多人互动，对于游戏反馈很主观）、游戏测试（游戏质量、程序bug、多平台兼容、新功能更新、性能，游戏测试比较系统客观）
- **软件测试和游戏测试的区别**：用户体验重要性（软件测试对于用户体验是次要的，保证功能即可；游戏测试需要注重用户体验，包括界面、音效、交互、游戏、沉浸式）、多媒体元素不同（游戏测试注重游戏渲染、音效等等）、测试类型（软件测试：功能、性能、回归；游戏测试有专门的游戏情景测试）、测试目标（软件测试关注功能的准确性、游戏测试关注用户的娱乐体验、趣味性）
- **云顶测试内容**：核心的游戏机制测试、英雄平衡测试、图形和性能、UI测试（分辨率）、音效和音乐测试、本地化测试（支持多语言）
- **自动化在游戏测试的作用**：自动化回归测试、快速执行大规模测试、性能测试（检查游戏在各种设备上的运行情况，以及不同网络状态下的性能）、场景测试（多人游戏、网络连接、特殊事件触发）
- **cin和getline输入字符串的区别**：cin默认使用空格作为分隔符，遇到空格就停止；getline允许指定分隔符，通常是换行，可以包含其中的空格
- **vector防止扩容的方法**：使用reverse预分配足够的空间；使用emplace_back直接在vector中构造元素，避免临时对象的创建和拷贝
- int float bool类型怎么和0比较：int直接比较；float由于精度问题应该使用范围比较

![image-20231120212428893](picture/Readme.assets/image-20231120212428893.png)
...
- 三个维度：map或者set、关键字是否重复、顺序或无序

![image-20231106164411758](picture/Readme.assets/image-20231106164411758.png)

- 为什么关联容器底层是红黑树而无序关联容器底层是哈希

1. **关联容器（基于红黑树）**:
   - **有序性**：关联容器保持元素的有序性，因为它们使用平衡二叉搜索树（通常是红黑树）来存储元素。**这意味着元素按照键的顺序排列，可以进行范围查找和范围遍历**。
   - 唯一性：关联容器确保元素是唯一的，不会包含重复的键值。
2. **无序关联容器（基于哈希表）**:
   - 散列性：**无序关联容器使用哈希表来存储元素，这使得元素的存储位置取决于其哈希值。这可以导致元素的存储顺序与插入顺序不同，因此元素在容器中没有特定的顺序**。
   - 查找性能：哈希表提供了常数时间的平均查找复杂度，这在某些情况下可以比红黑树更快。

- 管理桶：无序容器在存储上组织为一组桶，每个桶保存0个或多个元素，无序元素使用哈希函数将元素映射到桶，为了访问一个元素，容器首先计算元素的哈希值，指定搜索的桶；容器将具有一个哈希值的所有元素都保存在相同的桶中（性能依赖哈希函数的质量和桶的大小）

## 动态内存

- 最安全的分配和使用动态内存的方法是调用make_shared函数，返回该对象的shared_ptr

```c++
shared_ptr<int> p = std::make_shared<int>(1);
```

类似emplace成员，用其参数来构造给定类型的对象

- 为什么使用动态内存：不知道需要多少对象、不知道对象的准确类型、需要在多个对象间共享数据
- **一个类分配多个对象实例，当一个对象实例销毁，其数据也被销毁，要想保留下来，需要共享这些数据，可以为每一个对象设置一个shared_ptr来管理，因此shared_ptr的成员将记录有几个对象实例共享相同的数据，并在最后一个对象实例被销毁时释放数据内存**
- 内存耗尽：内存耗尽，new失效，会抛出异常；为了阻止抛出异常，使用**定位new表达式**向new传递额外的参数
- **不要使用内置指针来访问一个智能指针，无法知道对象何时被销毁**
- unique_ptr不支持拷贝和赋值，可以通过release或reset进行转移
- weak_ptr：绑定到shared_ptr不会改变其引用计数（弱共享）
- **可以管理new分配的数组的unique_ptr版本 unique_ptr<int[]> up(new int [10]);**
- shared_ptr不直接支持管理动态数组，必须自己定义删除器 shared_ptr<int> sp(new int [10],[] (int *p){delete []p})
...
1. **绘制的图像放在颜色缓存中（GPU）**
2. 两种颜色模式：**RGB和颜色索引模式**（所有颜色值组织为一个表格，通过索引来指定）
3. opengl坐标系原点在屏幕左下角，GLUT坐标系原点在屏幕左上角
4. 视口：限制屏幕窗口的一部分进行绘制，尺寸度量为像素；应用：对窗口进行划分（不同类型的信息绘制在不同的部分）
5. **坐标系变换：模型视图矩阵和投影矩阵**
6. 点绘制线

![image-20231106154624472](picture/Readme.assets/image-20231106154624472.png)

7.图元填充

![image-20231106155032938](picture/Readme.assets/image-20231106155032938.png)

绘制速度比传统直接绘制三角形或四边形要快

8.多边形类型：凸对象（链接任意两点，整个线段位于该对象内部）

9.颜色插值

10.离散处理与边标记

11.字体：**位图字体（位的矩阵）和笔划字体（基本图元绘制）**
...
- **在派生类中，虚函数表包括：基类的虚函数表部分、派生类新增的虚函数表部分**、

![image-20231122134607330](picture/Readme.assets/image-20231122134607330.png)



### 3、TCP拥塞控制流程

- **慢启动**：TCP发送方将拥塞窗口设置为一个较小的值，在每个往返时间内，拥塞窗口大小会翻倍，发送方发送的数据量随着时间呈指数增长，以便快速填充网络的带宽
- **拥塞避免**：拥塞窗口达到慢启动的阈值，就进入拥塞避免阶段，窗口是线性增长的，有助于维护网络的稳定性
- **快速重传**：如果发送方检测到连续的三个ACK，说明有一个报文段丢失，发送方会立即重传，拥塞窗口被减半，重新进入慢启动
- **快速恢复**：窗口由线性增长改为窗口增加一个报文段大小，每收到一个ACK就增加
...
- 隐式参数：非静态成员函数在被调用时，编译器会**隐式传递一个额外的参数，this指针**
- 指向对象：this指针**指向当前对象的地址**，在成员函数内部可以通过该指针访问类的成员变量
- 使用场景：**解决成员函数参数与成员变量命名冲突**
- 存储在**对象的内存空间**中

### 7、判断点在三角形内

- 计算点与三角形三个顶点向量与三个边向量叉积的方向，一致在三角形内，否则不在

![image-20231121141902103](picture/Readme.assets/image-20231121141902103.png)



### 8、C++和python的区别

- C++是静态类型语言，需要在编译时声明变量的类型，并在运行时严格检查类型；python是动态类型语言，变量的类型在运行时确定
- 语法难易
- 内存管理：C++需要手动管理内存；python通过垃圾回收机制自动管理内存
- 性能不同

### 9、垃圾回收机制

- 引用计数
- 标记-清除（解决循环引用）：标记所有可达对象，清除未标记对象
- 标记-整理：标记后将标记对象向一端移动，然后清理边界外的内存，保持内存连续性
- java：分代收集；python使用标记-清除和分代

# 小米二面

### 1、线程和进程切换的上下文资源

- 线程上下文切换：**寄存器**（线程的执行状态保存在寄存器中，当线程切换时，当前线程的寄存器状态需要保存，新线程的寄存器状态需要加载）；**栈**（线程的局部信息和函数调用信息）；**程序计数器**（PC）（指示当前执行的指令位置）
- 进程上下文切换：寄存器、栈、程序计数器、内存映像（进程的地址空间包括代码段、数据段、栈、堆）、进程控制块（包括进程状态、优先级、进程ID）
- **改进方法：减少上下文切换频率、使用轻量级线程、改进调度算法、使用无锁数据结构**
- **进程调度算法：先来先服务、最短作业优先（预估运行时间最短的作业）、优先级、时间片轮转、多级反馈队列（将进程分为多个队列，每个队列有不同的优先级）**
- **线程调度算法：抢占式和非抢占式、优先级、轮询**

### 2、CUDA编程

- 由NVIDIA推出的并行计算框架，用于GPU上进行并行计算
- GPU架构和线程模型（网络、块、线程的层次结构）
- CUDA核函数：每个核函数被每个线程调用一次，可以通过块和线程索引确定自己的工作
- 内存层次结构：全局内存、共享内存（每个块共享的内存）、常量内存（只读的全局内存）
- CUDA API
- CUDA编译器
- CUDA C++和矿建
...
- 默认成员访问权限不同
- 都包含成员变量、成员函数、构造函数、析构函数，支持多态和继承

### 2、单例模式实现

- 目的：确保一个类只有一个实例，提供全局访问点
- 懒汉式：第一次使用时创建实例（缺点：多线程的竟态）
- 饿汉式：在程序启动时就创建实例（天生线程安全）
...
public:
    // 获取单例实例的静态方法
    static Singleton& getInstance() {
        // 使用懒汉式（Lazy Initialization）方式，在需要时才创建实例
...
- 物理内核和逻辑内核：一个物理内核包括寄存器、执行单元；逻辑内核通过超线程计数模拟出来的虚拟处理单元，逻辑内核共享物理内核的资源，允许执行多个线程

### 6、lambda表达式底层实现

- 匿名函数
- 底层：闭包和仿函数
- 闭包：闭包允许函数对象在其创建的上下文中捕获和使用外部变量
- 仿函数：operator（）

### 7、shared_ptr是线程安全的吗

- 是，引用计数是一个原子操作
- 线程安全：在多线程下，对于共享资源的操作不会导致意外的结果
- 原子性、可见性（锁）、有序性（条件变量）
...
- 提高内存分配和释放
- 一次分配多次使用
- 减少内存碎片（可以控制内存的分配和释放，使得内存的分布更加紧凑）和系统调用
- 避免内存泄漏（可以释放整个内存池，不需要逐步释放内存块）
- 提高局部性：提高内存访问的局部性，分配时更有可能提供相邻的内存块，有助于CPU命中率 
- 减少锁的竞争：内存池的预分配策略可以减少对于锁的竞争
- 代码示例：boost.pool

### 9、动态库如何调用

- 创建dll
- 加载动态库、获取动态库中的函数、调用函数、卸载动态库
- 编译exe
- 运行

### 10、git 一个分支同步到另一个分支的一个commit命令

- git cheakout 目标分支
- git log 找到要同步的commit
- git checkout 源分支
- git cherry-pick commit

### 11、计数排序（稳定的）

- 非比较性排序算法，适用于待排序元素的**范围较小**的情况
- 先找到数组中最大和最小的元素，然后创建一个计数数组，统计每个元素的出现次数，根据计数数组重新构建原数组
...
int main() {
    std::vector<int> arr = {4, 2, 3, 1, 0, 4, 6, 5};
...
- 分布式排序算法，将数据分散到多个桶，对每个桶中的数据进行排序，最后顺序合并得到结果
...
using namespace std;
 void BucketSort(vector<int>&arr,int BucketSize){
     ...
     int count = (maxval-minval)/BucketSize+1;
     vector<vector<int>> buckets(count,vector<int>(BucketSize,0));
     for(int num:arr){
         int index = (num-minval)/BucketSize;
         buckets[index].push_back(num);
     }
     for(vector<int>& bucket:buckets) sort(bucket.begin(),bucket.end());
 }
...
```C++
std::vector<std::vector<int>> v(3, std::vector<int>(4, 0)); 
```

```C++
size_t rows = v.size();          // 获取行数
size_t cols = (rows > 0) ? v[0].size() : 0;  // 获取列数
```
...
- range分区：基于范围分区
- list分区：基于离散值分区
- hash分区：使用哈希函数将数据均匀分布
- key分区：类似hash分区，使用用户自定义的表达式进行哈希分区
- 提高查询效率，不需要扫描整个表，简化数据维护

### 2、静态和动态sql

- 静态sql是在编译时已知的，查询的结构和参数是固定的，不会在运行时发生变化；静态sql通过预编译的sql语句实现的
- 动态sql是在运行时构建的，查询结构和参数在程序运行时间可以改变；查询的组成部分可能会根据程序运行时的条件而变化；动态sql可能使用字符串拼接来查询
- 推荐静态sql，防止sql注入攻击；在数据库引擎有更好的优化，因为查询结构提前预编译了

### 3、mysql存储引擎

- **InnoDB**:ACID、行级锁、外键支持（**事务性应用、高并发**）

- **MyISAM**：不支持事务、表级锁、支持全文本搜索（**读密集、写相对较少的场景：博客网站、新闻网站**）

- MEMORY（HEAP）：数据存储在内存中、速度快、但是会在服务重启时丢失（临时场景）

  

### 4、MyISAM全文本搜索底层实现

- 分词（token）：文本数据被分词器拆分为单独的词汇
- 停用词过滤：and the一些常用词被过滤，减少索引数量
- 建立倒排索引（inverted index）：将词汇映射到文档行的数据结构，对于每个单词都会记录包含该单词的文档位置
- 权重计算：根据重要性排序
